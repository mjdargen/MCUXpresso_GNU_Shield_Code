...
uint32_t __attribute__((weak)) osRtxMemoryInit (void *mem, uint32_t size) {
  mem_head_t  *head;
  mem_block_t *ptr;
...
void __attribute__((weak)) *osRtxMemoryAlloc (void *mem, uint32_t size, uint32_t type) {
  mem_block_t *ptr;
  mem_block_t *p, *p_new;
  uint32_t     block_size;
  uint32_t     hole_size;
...
/// \return 1 - success, 0 - failure.
uint32_t __attribute__((weak)) osRtxMemoryFree (void *mem, void *block) {
